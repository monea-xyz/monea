...
fn main() {
    ...
    let kurtosis_pkg_src = Path::new(&cli_dir)
        .join("..")
        .join("..")
        .join("kurtosis-pkg");
    ...
    println!("kurtosis_pkg_dest: {}", kurtosis_pkg_dest.display());

    // Copy the kurtosis package files
    copy_kurtosis_pkg(&kurtosis_pkg_src, &kurtosis_pkg_dest);

    // Download and build Kurtosis
    // download_and_build_kurtosis(&kurtosis_dest);

    println!("cargo:rerun-if-changed=../../kurtosis-pkg");
}

fn copy_kurtosis_pkg(kurtosis_pkg_src: &Path, kurtosis_pkg_dest: &Path) {
    // Check if source directory exists
    if !kurtosis_pkg_src.exists() {
        eprintln!(
            "Error: Monea Kurtosis package source directory does not exist: {}",
            kurtosis_pkg_src.display()
        );
        std::process::exit(1);
    }

    // Remove previous copy of kurtosis package if it exists
    if kurtosis_pkg_dest.exists() {
        fs::remove_dir_all(&kurtosis_pkg_dest).unwrap_or_else(|e| {
            eprintln!(
                "Warning: Failed to remove existing kurtosis package directory from target: {}",
                e
            );
        });
    }
    ...
    fs::create_dir_all(&kurtosis_pkg_dest).unwrap_or_else(|e| {
        eprintln!(
            "Error: Failed to create kurtosis package directory in target: {}",
            e
        );
        std::process::exit(1);
    });
    ...
    // Copy the kurtosis package files
    match copy_dir_all(&kurtosis_pkg_src, &kurtosis_pkg_dest) {
        Ok(_) => println!("Successfully copied kurtosis package files to target"),
        Err(e) => {
            eprintln!("Error: Failed to copy kurtosis package files: {}", e);
            std::process::exit(1);
        }
    }
}
