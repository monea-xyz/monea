...
def run(plan, args):
    ...
    # add get_services check for L1 here

    # l1 = read_file(src = "./l1_config.json")
    # l1 = json.decode(read_file(src = "./l1_config.json"))
    # l1 = json.encode(read_file(src = "./l1_config.json"))
    # l1 = json.encode(json.decode(read_file(src = "./l1_config.json")))
    l1_config_string = read_file(src = "./l1_config.json")
    l1 = json.decode(l1_config_string)
    
    # # Get L1 info
    all_l1_participants = l1["all_participants"]
    l1_network_params = l1["network_params"]
    l1_network_id = l1["network_id"]
    # safely access the private key, providing a default if not found
    l1_priv_key = l1.get("pre_funded_accounts", [{}])[12].get("private_key", "")
    if not l1_priv_key:
        plan.print("warning: could not find private key for L2 contract deployer")
...
def get_l1_config(all_l1_participants, l1_network_params, l1_network_id):
    ...
    env_vars["WEB3_RPC_URL"] = str(all_l1_participants[0].get("el_context", {}).get("rpc_http_url", ""))
    env_vars["L1_RPC_URL"] = str(all_l1_participants[0].get("el_context", {}).get("rpc_http_url", ""))
    env_vars["CL_RPC_URL"] = str(all_l1_participants[0].get("cl_context", {}).get("beacon_http_url", ""))
    env_vars["L1_WS_URL"] = str(all_l1_participants[0].get("el_context", {}).get("ws_url", ""))
    env_vars["L1_CHAIN_ID"] = str(l1_network_id)
    env_vars["L1_BLOCK_TIME"] = str(getattr(l1_network_params, "seconds_per_slot", ""))
    return env_vars
